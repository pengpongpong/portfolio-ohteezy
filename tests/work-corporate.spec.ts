...
test.describe("Test corporate work page", () => {
    test.beforeEach(async ({ page }, testInfo) => {
        console.log(`Testing ${testInfo}`)

        await page.goto("http://localhost:4321/work/corporate")
    })
    ...
    test("should have corporate images", async ({ page }) => {
        await expect(page.getByRole('heading', { name: 'Corporate Design' })).toBeVisible();
        await expect(page.getByRole('img', { name: 'Cafe Brand Sketch' })).toBeVisible();
        await expect(page.getByRole('img', { name: 'Cafe Brand Design Poster' })).toBeVisible();
        await expect(page.locator('div').filter({ hasText: /^Von der Skizze \.\.\.$/ }).locator('#image1')).toBeVisible();
        await expect(page.getByRole('img', { name: 'Pizza brand sketch' })).toBeVisible();
        await expect(page.getByRole('img', { name: 'Pizza brand poster' })).toBeVisible();
        await expect(page.locator('#image1').nth(2)).toBeVisible();
        await expect(page.getByRole('img', { name: 'Pizza brand illustration' })).toBeVisible();
        await expect(page.locator('#image1').nth(3)).toBeVisible();
    })
